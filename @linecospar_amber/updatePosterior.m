function updatePosterior(obj,newDataInds,newLabel)
...
%         2) labels: num_data_points NumPy array (all elements should be
%         zeros
%            and ones)
%         3) GP_prior_cov_inv: n-by-n NumPy array, where n is the number of
%            points over which the posterior is to be sampled
%         4) preference_noise: positive scalar parameter. Higher values
%         indicate
%            larger amounts of noise in the expert preferences.
%
%     Output: the updated model posterior, represented as a dictionary of
%     the
%            form {'mean': post_mean, 'cov_evecs': evecs, 'cov_evals':
%            evals}; post_mean is the posterior mean, a length-n NumPy
%            array in which n is the number of points over which the
%            posterior is to be sampled. cov_evecs is an n-by-n NumPy array
%            in which each column is an eigenvector of the posterior
%            covariance, and evals is a length-n array of the eigenvalues
%            of the posterior covariance.
%%%%%%%%%%%%%%%%%%%%%%%%%%%

% add new data to list
obj.data_matrix_inds = [obj.data_matrix_inds;newDataInds];
obj.labels = [obj.labels;newLabel];

%%% Update Prior over visited points
[~, prior_cov_inv] = obj.calculatePrior(obj.visited_actions_a);


num_features = size(obj.visited_actions_a,1); %number of points in subspace
...
obj.posterior_model_F.mean = post_mean;
obj.posterior_model_F.cov_evecs = evecs;
obj.posterior_model_F.cov_evals = evals;
obj.posterior_model_F.actions = obj.visited_actions_a;
